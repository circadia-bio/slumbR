...
> \[!WARNING\] **slumbR is in early development and has not been
> formally tested.** The API may change without notice, scoring
> algorithms have not yet been validated against reference
> implementations, and the package has not undergone peer review. Use
> with caution and verify outputs independently before using in any
> research context.
...
slumbR is the official R companion to the [Sleep Diaries
app](https://sleepdiaries.circadia-lab.uk). When a study ends,
participants export their data as a JSON file. slumbR reads those files
— one per participant or a whole directory at once — and hands you a
clean, analysis-ready data frame in seconds.

It computes standard sleep variables (TIB, TST, SE, SOL, WASO) directly
from the raw diary answers, re-scores all eight validated questionnaires
bundled in the app (ESS, ISI, DBAS-16, MEQ, PSQI, RU-SATED, STOP-BANG,
MCTQ), and assembles both long and wide study-level frames ready for
downstream statistics or visualisation.
...
> \[!NOTE\] The bundled dataset is entirely **simulated**. The three
> participants (Sarah Chen, James Okonkwo, Priya Mehta) and all their
> diary entries and questionnaire responses are artificially generated
> for demonstration purposes. They do not represent real people or real
> study data.
...
> **Note:** Some instruments require institutional permission for
> research use. Please verify licensing before use. The app’s
> `Settings → Questionnaire Credits` screen lists the relevant
> permissions for each instrument.
...
- 🌙 [**SleepDiaries**](https://github.com/circadia-bio/SleepDiaries) —
  the cross-platform Sleep Diaries app that generates the exports slumbR
  reads
